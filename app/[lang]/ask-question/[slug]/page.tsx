...
const router = useRouter()
  ...
  const question = {
    ...
    likes: 23,
    isLiked: false,
    description: "Интересует возможность получения кредита без предоставления справки о доходах. Работаю неофициально, но имею стабильный доход. Какие есть варианты?",
    icon: "👤",
  ...
  const [answers, setAnswers] = useState([
    ...
    },
    {
      ...
      likes: 8,
      isLiked: true,
      answer: "Я получала такой кредит в Кредиторе. Нужен был только паспорт и ИНН. Правда, сумма была небольшая - 25 000 грн на 6 месяцев. Ставка 2,5% в месяц. Одобрили за 20 минут.",
      isHelpful: false,
      helpfulCount: 8,
      avatar: "👩"
    },
  ...
  const handleLikeAnswer = (answerId: number) => {
    setAnswers(answers.map(answer => 
      answer.id === answerId 
        ? { ...answer, isLiked: !answer.isLiked, likes: answer.isLiked ? answer.likes - 1 : answer.likes + 1 }
        : answer
    ));
  };
  ...
  const handleMarkHelpful = (answerId: number) => {
    setAnswers(answers.map(answer => 
      answer.id === answerId 
        ? { ...answer, isHelpful: !answer.isHelpful, helpfulCount: answer.isHelpful ? answer.helpfulCount - 1 : answer.helpfulCount + 1 }
        : answer
    ));
  };
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-8 mb-8">
          <div className="flex items-start gap-6">
            {/* Icon */}
            <div className={`w-16 h-16 bg-gradient-to-br ${question.color} rounded-2xl flex items-center justify-center text-white text-2xl flex-shrink-0`}>
              {question.icon}
            </div>
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className={`text-xs px-3 py-1 rounded-full font-medium bg-gradient-to-r ${question.color} text-white`}>
                  {question.category}
                </span>
              </div>
              ...
              <div className="flex items-center gap-6">
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all duration-200 ${question.isLiked 
                    ? 'bg-red-100 text-red-600' 
                    : 'bg-gray-100 text-gray-600 hover:bg-red-50 hover:text-red-500'
                  }`}
                >
                  <svg className="w-5 h-5" fill={question.isLiked ? "currentColor" : "none"} stroke="currentColor" viewBox="0 0 24 24">
            ...
            {answers.map((answer, index) => (
              ...
              >
                <div className="flex items-start gap-4">
                  {/* Avatar */}
                  <div className={`w-12 h-12 ${answer.isExpert ? 'bg-gradient-to-br from-green-500 to-green-600' : 'bg-gradient-to-br from-gray-500 to-gray-600'} rounded-xl flex items-center justify-center text-white text-xl flex-shrink-0`}>
                    {answer.avatar}
                  </div>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4">
                      <button
                        ...
                        className={`flex items-center gap-2 px-3 py-2 rounded-xl text-sm transition-all duration-200 ${answer.isLiked 
                          ? 'bg-blue-100 text-blue-600' 
                          : 'bg-gray-100 text-gray-600 hover:bg-blue-50 hover:text-blue-500'
                        }`}
                      >
                        <svg className="w-4 h-4" fill={answer.isLiked ? "currentColor" : "none"} stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      <button
                        ...
                        className={`flex items-center gap-2 px-3 py-2 rounded-xl text-sm transition-all duration-200 ${answer.isHelpful 
        ...
        {showAnswerForm && (
          <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
            <h3 className="text-xl font-bold text-gray-800 mb-4">Написать ответ</h3>
        ...
        {/* Related Questions */}
        <div className="mt-12">
          <h3 className="text-2xl font-bold text-gray-800 mb-6">Похожие вопросы</h3>
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-3">
                      <span className={`text-xs px-2 py-1 rounded-full font-medium bg-gradient-to-r ${item.color} text-white`}>
                        {item.category}
                      </span>
